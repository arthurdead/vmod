...
namespace vmod
{
	...
	bool compile_internal_script(gsdk::IScriptVM *vm, std::filesystem::path path, const unsigned char *data, gsdk::HSCRIPT &object, bool &from_file) noexcept
	{
		...
		if(std::filesystem::exists(path)) {
			...
			object = CompileScript_strict(vm, reinterpret_cast<const char *>(script_data.get()), path.c_str()).release();
			if(!object || object == gsdk::INVALID_HSCRIPT) {
				if(data) {
					...
					object = CompileScript_strict(vm, reinterpret_cast<const char *>(data), filename.c_str()).release();
					if(!object || object == gsdk::INVALID_HSCRIPT) {
