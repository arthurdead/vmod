...
::__ReplaceClosures <- function( script, scope )
{
	if ( !scope )
	{
		scope = ::getroottable();
...
//---------------------------------------------------------
::__DumpScope <- function( depth, table )
{
...
//---------------------------------------------------------
::__CollectEventCallbacks <- function( scope, prefix, globalTableName, regFunc )
{
...
::__CollectGameEventCallbacks <- function( scope )
{
...
//---------------------------------------------------------
::__RunEventCallbacks <- function( event, params, prefix, globalTableName, bWarnIfMissing )
{
	local useTable = ::getroottable()[globalTableName] 
...
::FireScriptEvent <- function( event, params )
{
...
::ScriptDebugDrawWatches <- function( line )
{
...
::ScriptDebugAddWatch <- function( watch )
{
...
::ScriptDebugAddWatches <- function( watchArray )
{
...
::ScriptDebugRemoveWatch <- function( watch )
{
...
::ScriptDebugRemoveWatches <- function( watchArray )
{
	if ( typeof( watchArray ) == "array" )
	{
		for ( local i = 0; i < watchArray.len(); i++ )
...
::ScriptDebugAddWatchPattern <- function( name )
{
	if ( name == "" )
	{
...
::ScriptDebugRemoveWatchPattern <- function( name )
{
	if ( name == "" )
	{
		::Msg( "Cannot find an empty string" );
...
::ScriptDebugClearWatches <- function()
{
...
::ScriptDebugTraceAll <- function( bValue = true )
{
	::ScriptDebugTraceAllOn = bValue;
};

::ScriptDebugAddTrace <- function( traceTarget )
{
...
::ScriptDebugRemoveTrace <- function( traceTarget )
{
...
::ScriptDebugClearTraces <- function()
{
	::ScriptDebugTraceAllOn = false;
	::ScriptDebugTraces.clear();
};

::ScriptDebugTextTrace <- function( text, color = [ 255, 255, 255 ] )
{
...
::ScriptDebugTextPrint <- function( text, color = [ 255, 255, 255 ], isWatch = false )
{
...
::ScriptDebugTextDraw <- function( line )
{
	local i;
	local alpha;
	local curtime = ::Time();
...
::ScriptDebugAddTextFilter <- function( filter )
{
...
::ScriptDebugRemoveTextFilter <- function( filter )
{
...
::ScriptDebugHook <- function( type, file, line, funcname )
{
